...
# Feature: feishu-ai-bot, Property 32: Gemini CLI 原生会话使用
# **Validates: Requirements 10.11, 10.13**
@settings(max_examples=100)
@given(user_id=user_id_strategy())
def test_gemini_cli_uses_native_session_when_enabled(user_id):
    """
    Property 32: Gemini CLI 原生会话使用
    
    For any 使用 Gemini CLI 的执行，当启用原生会话管理时，
    构造的命令应该包含 --session 参数和对应用户的 Gemini 会话 ID。
    
    **Validates: Requirements 10.11, 10.13**
    """
    directory_path = tempfile.mkdtemp(prefix="test_gemini_session_")
    session_storage_path = os.path.join(directory_path, "executor_sessions.json")
    try:
        # 创建 Gemini CLI 执行器，启用原生会话
        ...
        # 模拟设置会话 ID
        test_session_id = f"gemini_session_{user_id}"
...
@settings(max_examples=100)
@given(user_id=user_id_strategy())
def test_gemini_cli_no_session_when_disabled(user_id):
    """
    Property 32: Gemini CLI 原生会话使用 - 禁用时不使用会话
    
    For any 使用 Gemini CLI 的执行，当禁用原生会话管理时，
    构造的命令不应该包含 --session 参数。
    
    **Validates: Requirements 10.11**
    """
    directory_path = tempfile.mkdtemp(prefix="test_gemini_no_session_")
    session_storage_path = os.path.join(directory_path, "executor_sessions.json")
    try:
        # 创建 Gemini CLI 执行器，禁用原生会话
        executor = GeminiCLIExecutor(
            target_dir=directory_path,
            timeout=5,
...
# Feature: feishu-ai-bot, Property 34: Gemini CLI 会话映射
...
def test_gemini_cli_maintains_session_mapping(user_id, session_id):
    """
    Property 34: Gemini CLI 会话映射
    
    For any 用户 ID，Gemini Executor 应该维护一个从用户 ID 到 Gemini 会话 ID 的映射，
    确保同一用户的多次请求使用相同的 Gemini 会话。
    
    **Validates: Requirements 10.13**
    """
    directory_path = tempfile.mkdtemp(prefix="test_gemini_mapping_")
    session_storage_path = os.path.join(directory_path, "executor_sessions.json")
    try:
        # 创建 Gemini CLI 执行器
        executor = GeminiCLIExecutor(
        ...
        # 首次获取会话 ID（应该为 None）
        initial_session_id = executor.get_or_create_gemini_session(user_id)
        ...
        # 再次获取会话 ID（应该是更新后的值）
        retrieved_session_id = executor.get_or_create_gemini_session(user_id)
        assert retrieved_session_id == session_id, \
            f"Retrieved session ID should be {session_id}, got {retrieved_session_id}"
        
        # 多次获取应该返回相同的会话 ID
        for _ in range(5):
            current_session_id = executor.get_or_create_gemini_session(user_id)
...
def test_gemini_cli_isolates_user_sessions(user_id_1, user_id_2, session_id_1, session_id_2):
    """
    Property 34: Gemini CLI 会话映射 - 用户会话隔离
    
    For any 两个不同的用户 ID，它们的 Gemini 会话应该是独立的，
    ...
    **Validates: Requirements 10.13**
    ...
    directory_path = tempfile.mkdtemp(prefix="test_gemini_isolation_")
    session_storage_path = os.path.join(directory_path, "executor_sessions.json")
    try:
        # 创建 Gemini CLI 执行器
        executor = GeminiCLIExecutor(
        ...
        # 验证每个用户的会话 ID 独立
        retrieved_session_1 = executor.get_or_create_gemini_session(user_id_1)
        retrieved_session_2 = executor.get_or_create_gemini_session(user_id_2)
...
def test_gemini_cli_clear_session(user_id, session_id):
    """
    Property 34: Gemini CLI 会话映射 - 清除会话
    
    For any 用户 ID，调用 clear_session 后，该用户的会话映射应该被删除。
    
    **Validates: Requirements 10.13**
    """
    directory_path = tempfile.mkdtemp(prefix="test_gemini_clear_")
    session_storage_path = os.path.join(directory_path, "executor_sessions.json")
    try:
        # 创建 Gemini CLI 执行器
        executor = GeminiCLIExecutor(
        ...
        cleared_session_id = executor.get_or_create_gemini_session(user_id)
