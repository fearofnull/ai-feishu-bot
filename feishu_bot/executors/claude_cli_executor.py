...
执行 Claude Code CLI 命令，支持 Claude Code 的原生会话管理。
"""
import platform
import subprocess
import time
...
class ClaudeCodeCLIExecutor(AICLIExecutor):
    ...
    执行 Claude Code CLI 命令，支持：
    - 目录上下文管理（--add-dir）
    - 原生会话管理（--session）
    - 用户会话映射（user_id -> claude_session_id）
    - 会话持久化存储
    
    Requirements: 3.3, 3.4, 3.5, 3.6, 3.7, 3.8, 3.9, 3.10, 10.11, 10.12, 10.14
    """
    ...
    def get_or_create_claude_session(self, user_id: str) -> Optional[str]:
        ...
        首次执行时会话 ID 为 None，Claude Code 会自动生成。
        
        Args:
            user_id: 用户 ID
            
        Returns:
            Claude 会话 ID，如果是首次执行则返回 None
        """
    ...
    def build_command_args(
        ...
        Args:
            user_prompt: 用户提示
            additional_params: 额外参数（user_id, model, max_tokens 等）
        ...
        try:
            ...
            if self.use_native_session and additional_params:
                ...
                if user_id and result.returncode == 0:
                    # 尝试从输出中提取会话 ID
                    # 注意：这里假设 Claude Code 会在某处输出会话 ID
    ...
    def _try_extract_session_id(self, user_id: str, output: str) -> None:
        """尝试从 Claude Code 输出中提取会话 ID
        
        Args:
            user_id: 用户 ID
            output: Claude Code 输出
        """
        # TODO: 根据 Claude Code 的实际输出格式实现会话 ID 提取
        # 这里是一个占位实现
        pass
    ...
    def save_session_mappings(self) -> None:
        ...
        try:
            ...
            with FileLock(lock_path, timeout=10):
                ...
                # 更新 Claude CLI 会话映射
                data['claude_cli_sessions'] = self.session_map
            ...
            # 记录错误但不抛出异常，避免影响主流程
            print(f"保存 Claude CLI 会话映射失败: {e}")
    ...
    def load_session_mappings(self) -> None:
        ...
        try:
            if os.path.exists(self.session_storage_path):
                with open(self.session_storage_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.session_map = data.get('claude_cli_sessions', {})
        except Exception as e:
            # 记录错误但不抛出异常，使用空映射继续
            print(f"加载 Claude CLI 会话映射失败: {e}")
            self.session_map = {}
