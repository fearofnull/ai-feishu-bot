...
# 配置 SSL 证书
from src.xagent.utils.ssl_config import configure_ssl
configure_ssl()
...
def cleanup_expired_sessions(bot: XAgent) -> None:
    """清理过期会话的定时任务"""
    try:
...
def run_scheduler(bot: XAgent) -> None:
    """运行定时任务调度器"""
    # 每小时清理一次过期会话
...
def main():
    ...
    try:
        ...
        # 创建机器人实例
        logger.info("Initializing XAgent...")
        bot = XAgent(config)
