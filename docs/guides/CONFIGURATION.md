...
- 普通对话 → 使用 Web 管理界面配置的默认提供商（Agent）
...
- **默认值**: 无
- **可选值**: 无（已废弃）
- **说明**: 此配置项已废弃，请使用 Web 管理界面配置默认提供商
...
#### ENABLE_INPUT_SECURITY_AUDIT
- **类型**: 布尔值（true/false）
- **必需**: 否
- **默认值**: true
- **说明**: 是否启用输入安全审计（提示词拦截）
- **使用场景**: 
  - 启用: 对用户输入进行安全审计，拦截可能的恶意提示词
  - 禁用: 跳过输入安全审计，可能提高响应速度但降低安全性
- **注意**: 建议在生产环境保持启用状态

#### ENABLE_OUTPUT_SECURITY_FILTER
- **类型**: 布尔值（true/false）
- **必需**: 否
- **默认值**: true
- **说明**: 是否启用输出安全过滤（敏感信息检测和脱敏）
- **使用场景**: 
  - 启用: 检测并脱敏输出中的敏感信息（如API密钥、密码等）
  - 禁用: 跳过输出安全过滤，可能提高响应速度但降低安全性
- **注意**: 建议在生产环境保持启用状态
...
配置 Web 管理界面的默认提供商后，直接发送消息即可进行代码操作。
...
| 命令前缀 | AI 提供商 | 说明 |
|---------|----------|------|
| 无前缀 | Agent | 使用 Web 管理界面配置的默认提供商（默认方式） |
| `@agent` | Agent | 显式指定使用 Agent |
